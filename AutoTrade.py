...
# 시작 메세지 슬랙 전송
send_message("#crypto", "autotrade start")
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):    #끝나는시간을 다음날 9시에서 10초를 빼준 값으로 설정 (안하면 무한루프)
            target_price = get_target_price("KRW-BTC", 0.23) 
            ...
            if target_price < current_price and ma15 < current_price: 
                ...
                if krw > 5000:
                    ...
                    send_message("#crypto", "BTC buy : " +str(buy_result))
        else:
            btc = get_balance("BTC")
            if btc > 0.00006:
                sell_result = upbit.sell_market_order("KRW-BTC", btc*0.9995)
                send_message("#crypto", "BTC buy : " +str(sell_result))
        time.sleep(1)
    except Exception as e:
        print(e)
        send_message("#crypto", e)
        time.sleep(1)